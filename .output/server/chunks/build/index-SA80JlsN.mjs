...
import { u as useHead, a as useNuxtData, r as reactivePick, c as useRuntimeConfig } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen py-20 p-6 lg:py-18 relative overflow-hidden" }, _attrs))} data-v-963892cf><div class="absolute top-0 right-0 w-150 h-150 bg-cyan-400 rounded-full blur-[200px] opacity-[0.03]" data-v-963892cf></div><header class="max-w-350 mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6 relative z-10" data-v-963892cf><div data-v-963892cf><div class="flex items-center gap-3 mb-2" data-v-963892cf><div class="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_#30cf43]" data-v-963892cf></div><span class="text-[10px] font-black uppercase tracking-[0.4em] text-cyan-400" data-v-963892cf>Ghost-Node Network</span></div><h1 class="text-4xl md:text-5xl font-bold tracking-tighter" data-v-963892cf>${ssrInterpolate(unref(user).organization)}</h1><span class="font-bold tracking-tighter" data-v-963892cf>${ssrInterpolate(unref(user)?.category)}</span></div><div class="flex gap-4" data-v-963892cf><button class="bg-white/5 border border-white/10 px-6 py-3 rounded-xl text-xs font-bold hover:bg-white/10 transition-all backdrop-blur-md" data-v-963892cf> Global Analytics </button><button class="bg-cyan-400 text-black px-6 py-3 rounded-xl text-xs font-bold hover:shadow-[0_0_20px_rgba(48,207,67,0.4)] transition-all" data-v-963892cf> + New Capture Node </button></div></header><main class="max-w-350 mx-auto relative z-10" data-v-963892cf><section class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12" data-v-963892cf><!--[-->`);
      ssrRenderList({ "Total Intake": "1,284", "Conversion": "24.2%", "Active Nodes": "42", "AI Confidence": "98%" }, (val, label) => {
        _push(`<div class="backdrop-blur-xl bg-white/2 border border-white/8 p-8 rounded-3xl" data-v-963892cf><p class="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-4" data-v-963892cf>${ssrInterpolate(label)}</p><p class="text-3xl font-bold tabular-nums" data-v-963892cf>${ssrInterpolate(val)}</p></div>`);
      });
      _push(`<!--]--></section><div class="grid grid-cols-1 lg:grid-cols-12 gap-8" data-v-963892cf><div class="lg:col-span-8 space-y-6" data-v-963892cf><div class="flex justify-between items-end mb-4" data-v-963892cf><h2 class="text-lg font-bold" data-v-963892cf>Individual Personnel Tracking</h2><span class="text-[10px] text-zinc-500 uppercase tracking-widest font-mono" data-v-963892cf>Real-time Sync</span></div><div class="backdrop-blur-xl bg-white/2 border border-white/8 rounded-[2.5rem] overflow-hidden" data-v-963892cf><table class="w-full text-left" data-v-963892cf><thead class="bg-white/3 text-[10px] uppercase tracking-widest text-zinc-500" data-v-963892cf><tr data-v-963892cf><th class="p-6" data-v-963892cf>Agent Entity</th><th class="p-6 text-center" data-v-963892cf>Status</th><th class="p-6 text-center" data-v-963892cf>Intake</th><th class="p-6 text-right" data-v-963892cf>Actions</th></tr></thead><tbody class="divide-y divide-white/5" data-v-963892cf><!--[-->`);
      ssrRenderList(unref(agents), (agent) => {
        _push(`<tr class="hover:bg-white/2 transition-colors group" data-v-963892cf><td class="p-6" data-v-963892cf><p class="font-bold text-lg leading-none mb-1" data-v-963892cf>${ssrInterpolate(agent.name)}</p><p class="text-xs text-zinc-500" data-v-963892cf>${ssrInterpolate(agent.role)}</p></td><td class="p-6" data-v-963892cf><div class="flex justify-center" data-v-963892cf><span class="${ssrRenderClass([agent.status === "ACTIVE" ? "bg-cyan-400/20 text-cyan-400" : "bg-zinc-800 text-zinc-500", "text-[9px] font-black px-3 py-1 rounded-full"])}" data-v-963892cf>${ssrInterpolate(agent.status)}</span></div></td><td class="p-6 text-center" data-v-963892cf><p class="font-mono text-sm" data-v-963892cf>${ssrInterpolate(agent.leads)}</p><p class="text-[8px] text-cyan-400 font-bold" data-v-963892cf>Tier 1: ${ssrInterpolate(agent.tier1)}</p></td><td class="p-6 text-right" data-v-963892cf><button class="opacity-0 group-hover:opacity-100 transition-opacity bg-white/10 p-2 rounded-lg hover:bg-cyan-400 hover:text-black" data-v-963892cf><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" data-v-963892cf><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z" data-v-963892cf></path></svg></button></td></tr>`);
      });
      _push(`<!--]--></tbody></table></div></div><div class="lg:col-span-4 space-y-6" data-v-963892cf><h2 class="text-lg font-bold" data-v-963892cf>Company Asset Generator</h2><div class="backdrop-blur-xl bg-white/2 border border-white/8 rounded-[2.5rem] p-8" data-v-963892cf><p class="text-xs text-zinc-400 mb-8 leading-relaxed" data-v-963892cf> Generate a high-velocity QR node for physical signage (Yard Signs, Open Houses). Leads will be analyzed by AI and dispatched to the global queue. </p><div class="space-y-4" data-v-963892cf><input type="text" placeholder="Site Name (e.g. Whitefish Ridge)" class="w-full bg-white/5 border border-white/10 p-4 rounded-xl text-sm focus:border-cyan-400/50 focus:outline-none transition-all" data-v-963892cf><button class="w-full bg-linear-to-r from-cyan-400 to-[#15a327] text-black font-bold py-4 rounded-xl shadow-[0_0_30px_rgba(48,207,67,0.2)] hover:scale-[1.02] transition-all" data-v-963892cf> Generate Site Node </button></div><div class="mt-8 pt-8 border-t border-white/5 flex items-center justify-between" data-v-963892cf><span class="text-[10px] font-bold text-zinc-500 uppercase" data-v-963892cf>Recent Assets</span><div class="flex -space-x-2" data-v-963892cf><!--[-->`);
      ssrRenderList(3, (i) => {
        _push(`<div class="w-8 h-8 rounded-lg bg-zinc-800 border border-white/10 flex items-center justify-center text-[8px]" data-v-963892cf>QR</div>`);
      });
      _push(`<!--]--></div></div></div>`);
      _push(ssrRenderComponent(_component_baseQrCode, { value: "www.mdreesen.com" }, null, _parent));
      _push(`</div></div></main><footer class="max-w-350 mx-auto mt-24 border-t border-white/5 pt-8 flex justify-between items-center text-[10px] font-bold text-zinc-600 uppercase tracking-[0.4em]" data-v-963892cf><span data-v-963892cf>GhostForm // Global Lead Archive</span><span class="font-mono" data-v-963892cf>Status: Spectral-Encrypted</span></footer></div>`);
    };
  }
});
...
const index = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-963892cf"]]);

export { index as default };
//# sourceMappingURL=index-SA80JlsN.mjs.map
